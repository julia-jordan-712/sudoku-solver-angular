...
describe(VerifySolutionService.name, () => {
  ...
  [
    ...
    it(`should recognize a valid solution if only uniqueness is checked: ${params.desc}`, () => {
      expect(underTest.verify(params.input, {size: params.input.length}).isValid()).toBeTrue();
      expect(square.verify).not.toHaveBeenCalled()
      expect(uniqueness.verify).toHaveBeenCalledTimes(1)
    });

    it(`should recognize a valid solution if duplicates are not tracked: ${params.desc}`, () => {
      expect(underTest.verify(params.input, {trackUniquenessViolations: false}).isValid()).toBeTrue();
      expect(square.verify).toHaveBeenCalledTimes(1)
      expect(uniqueness.verify).toHaveBeenCalledOnceWith(params.input, params.input.length, {trackUniquenessViolations: false})
    });

    it(`should recognize a valid solution if duplicates are tracked: ${params.desc}`, () => {
      expect(underTest.verify(params.input, {trackUniquenessViolations: true}).isValid()).toBeTrue();
      expect(square.verify).toHaveBeenCalledTimes(1)
      expect(uniqueness.verify).toHaveBeenCalledOnceWith(params.input, params.input.length, {trackUniquenessViolations: true})
    });
  });
});
